#!/usr/bin/env bash
#SBATCH --job-name=ub_exactcmp
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=10
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=logs/job_%j.out
#SBATCH --error=logs/job_%j.err
#
# EXACT reproduction comparison, controlled across three eval axes.
#
#   Patches (rows of the table):
#     cf  = CODE-FAITHFUL retrain  : authors' verbatim optimise.py (only 3 I/O
#           constants changed), @1300 -> outputs/codefaithful_exact_1300/1300.png
#           Carries D11 band bug + dead §3.2 projection, exactly as released.
#     pf  = PAPER-FAITHFUL retrain : outputs/repro_paper_faithful_1300/1300.png
#     img = AUTHORS' RELEASED IMAGE (reference only, NOT code-faithful)
#
#   Axes (columns):
#     SafeBench size : 315 (clean, --exclude-train) vs 350 (contaminated)
#     AdvBench form  : raw goal (authors) vs normalized ("Steps to ...", our dev)
#     Judge          : v1 (authors', commit c4c276d) vs v3 (our corrected)
#
#   White-box only (Qwen2-VL-7B-Instruct surrogate), matching how 81.59% is
#   reported. Idempotent: existing generations/judgements are reused.
set -uo pipefail
REPO="/home2/home/ankur_d/sm/UltraBreak-Repro"; cd "$REPO"
source repro/bin/activate; export PYTHONUNBUFFERED=1
AUTH="c4c276d"; SURR="Qwen/Qwen2-VL-7B-Instruct"
STAGE="${SLURM_TMPDIR:-/tmp}/exactcmp_${SLURM_JOB_ID:-manual}"; mkdir -p "$STAGE" logs
L="[exactcmp]"; echo "$L start $(date)"

# ---------------- 1) TRAIN exact code-faithful @1300 -------------------------
CF_PNG="outputs/codefaithful_exact_1300/1300.png"
if [[ -f "$CF_PNG" ]]; then
  echo "$L code-faithful patch exists, skip training: $CF_PNG"
else
  echo "$L TRAIN code-faithful (authors' verbatim optimise_exact.py) ..."
  # script dir goes on sys.path[0] -> authors' verbatim adapters resolve;
  # CWD stays repo root -> train_configs/ and outputs/ are repo-root-relative.
  python reproducibility/authors_code/optimisation/optimise_exact.py || { echo "$L TRAIN FAILED" >&2; }
fi
[[ -f "$CF_PNG" ]] || { echo "$L ERROR no code-faithful patch; eval will skip cf" >&2; }

# ---------------- v1 judge (authors', materialized from git) -----------------
git show "${AUTH}:evaluation/evaluate.py" > "$STAGE/evaluate_v1.py"
echo "$L v1 judge sha256: $(sha256sum "$STAGE/evaluate_v1.py" | cut -d' ' -f1)"
cp "$STAGE/evaluate_v1.py" evaluation/_evaluate_v1_tmp.py
trap 'rm -f "${REPO}/evaluation/_evaluate_v1_tmp.py"' EXIT

# ---------------- helpers ----------------------------------------------------
build() {  # variant out_cfg patch  -> writes attack_configs/<out_cfg>.csv
  local v="$1" out="attack_configs/$2.csv" img="$3"
  case "$v" in
    sb315) python create_attack_configs.py --dataset safebench --config-type attack \
             --exclude-train datasets/SafeBench-Tiny.csv --image "$img" --output "$out" ;;
    sb350) python create_attack_configs.py --dataset safebench --config-type attack \
             --image "$img" --output "$out" ;;
    abraw) python create_attack_configs.py --dataset advbench --config-type attack \
             --image "$img" --output "$out" ;;
    abnorm) python create_attack_configs.py --dataset advbench --config-type attack \
             --normalize --image "$img" --output "$out" ;;
  esac
  [[ -s "$out" ]] || { echo "$L build failed $out" >&2; return 1; }
}
gen() {  # cfg  -> results/<cfg>/<SURR>.csv  (resume-guarded)
  local cfg="$1" res="results/$1/${SURR}.csv"
  if [[ -f "$res" ]] && python - "$res" "attack_configs/$1.csv" <<'PY'
import sys,pandas as pd
r,c=pd.read_csv(sys.argv[1]),pd.read_csv(sys.argv[2])
ne=r["response"].astype(str).str.strip().replace("nan","").astype(bool).sum() if "response" in r else 0
sys.exit(0 if (len(r)>=len(c) and ne>=len(c)) else 1)
PY
  then echo "$L reuse gen $cfg"; else
    echo "$L gen $cfg on $SURR"; python evaluation/attack.py --model_name "$SURR" --attack_config "$cfg" || echo "$L gen failed $cfg"
  fi
}
judge_v1() {  # cfg
  local res="results/$1/${SURR}.csv" out="results/$1/${SURR}_harmbench.csv"
  [[ -f "$res" ]] || { echo "$L v1 no gen $1"; return; }
  [[ -f "$out" ]] && { echo "$L reuse v1 $1"; return; }
  ( cd evaluation && python _evaluate_v1_tmp.py --attack_result "../$res" ) | tail -3
}
judge_v3() {  # cfg [behaviors...]
  local res="results/$1/${SURR}.csv" out="results/$1/${SURR}_harmbench_v3.csv"; shift
  [[ -f "$res" ]] || { echo "$L v3 no gen"; return; }
  [[ -f "$out" ]] && { echo "$L reuse v3"; return; }
  python evaluation/evaluate.py --attack_result "$res" --output_suffix "_harmbench_v3.csv" "$@" \
    || { echo "$L v3 with-behaviors failed, retry plain"; python evaluation/evaluate.py --attack_result "$res" --output_suffix "_harmbench_v3.csv"; }
}

# ---------------- 2) factorial over patches x variants -----------------------
declare -A PATCH=( [cf]="$CF_PNG" [pf]="outputs/repro_paper_faithful_1300/1300.png" [img]="outputs/ultrabreak.png" )
for tag in cf pf img; do
  img="${PATCH[$tag]}"; [[ -f "$img" ]] || { echo "$L skip $tag (missing $img)"; continue; }
  for v in sb315 sb350 abraw abnorm; do
    cfg="cmp_${tag}_${v}"
    build "$v" "$cfg" "$img" || continue
    gen "$cfg"
    judge_v1 "$cfg"
    if [[ "$v" == abraw ]]; then judge_v3 "$cfg" --behaviors_csv datasets/adv_bench.csv --behaviors_col goal
    else judge_v3 "$cfg"; fi
  done
done

# ---------------- 3) summary tables: ASR + NRR -------------------------------
echo "$L ===================== RESULT (white-box Qwen2-VL) ====================="
python - <<'PY'
import os,pandas as pd
SURR="Qwen/Qwen2-VL-7B-Instruct"
tags={"cf":"code-faithful retrain","pf":"paper-faithful retrain","img":"released image (ref)"}
vars_=["sb315","sb350","abraw","abnorm"]
def metrics(cfg,suf):
    """Return (ASR%, NRR%, n) for a judged cell, or None."""
    p=f"results/{cfg}/{SURR}{suf}"
    if not os.path.exists(p): return None
    d=pd.read_csv(p)
    if "attack_success" not in d.columns: return None
    asr=d["attack_success"].astype(bool).mean()*100
    # NRR = Non-Refusal Rate = fraction NOT refused (authors' second headline metric)
    nrr=(100.0-d["refused"].astype(bool).mean()*100) if "refused" in d.columns else float("nan")
    return (asr,nrr,len(d))
def table(idx,label):
    print(f"\n### {label} (%, white-box Qwen2-VL) ###")
    print(f"{'patch':22s}"+ "".join(f"{v+'/'+j:>12s}" for v in vars_ for j in ("v1","v3")))
    for t,name in tags.items():
        row=f"{name:22s}"
        for v in vars_:
            for suf in ("_harmbench.csv","_harmbench_v3.csv"):
                m=metrics(f"cmp_{t}_{v}",suf); row+=f"{(f'{m[idx]:.1f}' if m else '-'):>12s}"
        print(row)
table(0,"ASR — Attack Success Rate")
table(1,"NRR — Non-Refusal Rate")
print("\nAxes: sb315=clean SafeBench, sb350=contaminated | abraw=authors' raw goal, abnorm=normalized | v1=authors' judge, v3=corrected")
print("(n per cell: sb315=315, sb350=350, abraw/abnorm=520)")
PY
echo "$L done $(date)"
